...
export const get_next_page = (): void => {
  const current_table: number = parseInt(sessionStorage.getItem('t_table_number') || '0', 10);
  const current_page: number = parseInt(sessionStorage.getItem('t_table_pagination') || '0', 10);
...
export const load_t_t1_data_last_page = (): void => {
  const current_page: number = parseInt(sessionStorage.getItem('t_table_pagination') || '0', 10);
  ...
  $.ajax({
    ...
    cache: false,
    crossDomain: true,
    success: function (response: { number_of_page: string; }): void {
      const number_of_page: number = parseInt(response.number_of_page, 10);
      sessionStorage.setItem('last_page', response.number_of_page);
      const total: number = parseInt(sessionStorage.getItem('count_rows') || '0', 10);
      const next_page: number = current_page + 1;
...
export const count_t_t1_data = (): void => {
  $.ajax({
...
export const load_t_t1_table = (): void => {
...
export const load_t_t1_data = (current_page: number) => {
  $.ajax({
    ...
    success: (response: TT1[]) => {
      $('#loading').remove();

      let table_row = "";
      try {
        const response_array: TT1[] = response;
...
export const load_t_t2_data_last_page = (): void => {
...
export const count_t_t2_data = (): void => {
  var c1 = sessionStorage.getItem('load_t_t2_c1');
...
export const load_t_t2_table = (): void => {
...
export const load_t_t2_data = (current_page: number) => {
  var c1 = sessionStorage.getItem('load_t_t2_c1') ?? '';
  ...
  $.ajax({
    ...
    success: function (response: any) {
      ...
      var table_row = "";
      try {
        const response_array: TT2[] = response;
        console.log(response_array);
        if (response_array.length) {
          response_array.forEach((item, index) => {
            if (item.message) {
...
export const load_t_t2 = (param: string) => {
