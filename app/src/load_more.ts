...
export const get_next_page = (): void => {
  const current_page: number = parseInt(sessionStorage.getItem('accounts_table_pagination') || '0', 10);
  const total: number = parseInt(sessionStorage.getItem('count_rows') || '0', 10);
  const last_page: number = parseInt(sessionStorage.getItem('last_page') || '0', 10);
...
export const load_accounts_last_page = (): void => {
  var employee_no = sessionStorage.getItem('employee_no_search');
  var full_name = sessionStorage.getItem('full_name_search');
  var user_type = sessionStorage.getItem('user_type_search');
  const current_page: number = parseInt(sessionStorage.getItem('accounts_table_pagination') || '0', 10);
  $.ajax({
    ...
    success: function (response: { number_of_page: string; }) {
...
export const search_accounts = (current_page: number) => {
