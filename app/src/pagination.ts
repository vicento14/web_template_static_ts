...
let search_accounts_ajax_in_progress: boolean = false;

export const th_order_by = (order_by_code: number): void => {
  const current_page: number = parseInt((document.getElementById("accounts_table_pagination") as HTMLInputElement).value.trim());
...
export const get_prev_page = (): void => {
  const current_page: number = parseInt(sessionStorage.getItem('accounts_table_pagination') || '0');
...
export const get_next_page = (): void => {
  const current_page: number = parseInt(sessionStorage.getItem('accounts_table_pagination') || '0');
  const order_by_code: number = parseInt(sessionStorage.getItem('order_by_code') || '0');
  const total: number = parseInt(sessionStorage.getItem('count_rows') || '0');
...
export const count_accounts = (): void => {
  ...
  $.ajax({
    ...
    success: function (response: { total: string; }) {
...
export const load_accounts_pagination = (): void => {
  const employee_no = sessionStorage.getItem('employee_no_search');
  const full_name = sessionStorage.getItem('full_name_search');
  const user_type = sessionStorage.getItem('user_type_search');
  const current_page: number = parseInt(sessionStorage.getItem('accounts_table_pagination') || '0');
  ...
  $.ajax({
    ...
    success: (response: any) => {
      let page_options: string = "";
      try {
        const response_array: any[] = response; // Assuming response is an array
      ...
      const last_page_check: string = document.getElementById("accounts_table_paginations")?.innerHTML || '';
      if (last_page_check !== '') {
        const last_page: string = document.getElementById("accounts_table_paginations")?.lastChild?.textContent || '';
...
export const search_accounts = (current_page: number, order_by_code: number) => {
  ...
  const employee_no_input = document.getElementById('employee_no_search') as HTMLInputElement;
  const full_name_input = document.getElementById('full_name_search') as HTMLInputElement;
  const user_type_input = document.getElementById('user_type_search') as HTMLSelectElement;

  let employee_no: string = employee_no_input.value;
  let full_name: string = full_name_input.value;
  let user_type: string = user_type_input.value;

  const employee_no_1: string | null = sessionStorage.getItem('employee_no_search');
  const full_name_1: string | null = sessionStorage.getItem('full_name_search');
  const user_type_1: string | null = sessionStorage.getItem('user_type_search');
  ...
  $.ajax({
    ...
    success: function (response: UserAccount[]) {
      ...
      let headerIndex: HTMLElement;

      const cTh = document.getElementById("c_th") as HTMLElement;
      const employeeNoTh = document.getElementById("employee_no_th") as HTMLElement;
      const usernameTh = document.getElementById("username_th") as HTMLElement;
      const fullnameTh = document.getElementById("fullname_th") as HTMLElement;
      const sectionTh = document.getElementById("section_th") as HTMLElement;
      const roleTh = document.getElementById("role_th") as HTMLElement;
